...
(function () {
  ...
  const MonthlyHours = {
    _cache:   {},   // key='YYYY-M' → { [employeeNumber]: hoursDecimal }
    ...
    async getData(year, month) {
      ...
      const p = Api.fetchShifts(start, end).then((records) => {
        ...
        const result = {};
        Object.entries(byNum).forEach(([num, min]) => {
          result[num] = Math.round((min / 60) * 100) / 100;
        });
        this._cache[k] = result;
        delete this._pending[k];
        log('月間時間集計 完了', { key: k, 人数: Object.keys(result).length });
        return result;
      }).catch((e) => {
        err('月間時間集計失敗', e);
        delete this._pending[k];
    ...
    getPersonHoursText(employeeNumber) {
      const h = this._currentData[employeeNumber];
